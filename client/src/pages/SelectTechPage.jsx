...
try {
    ...
    // Get selected technology names for ML
    const selectedTechNames = technologies
    .filter(tech => selectedTechs.includes(tech._id))
    .map(tech => tech.name);

    // Navigate with user preferences
    navigate(`/roadmap/${response.data._id}`, {
    state: {
        userPreferences: {
        technology: selectedTechNames[0], // Use first selected tech
...
// // Auto navigate when all preferences are selected
// useEffect(() => {
// if (
...
//     });
// }
// }, [selectedTechs, skillLevel, schedule, navigate]);
